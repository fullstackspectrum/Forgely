...
**Captured 2026-08-14** against `full-stack-spectrum`, cold cache, `FORGELY_PERF_PAYLOAD=1`, `MAX_WORKERS=20`.
...
|---|---|---|---|---|---|---|---|---|---|
| `neuro-containers-build` | 18 | 5.2s | 39 | 18 | 2 | 18 / 18 | 19 | 19 | 2.8 MB |
| `neuro-containers` | 214 | 11.9s | 490 | 214 | 59 | 214 / 214 | 215 | 1,294 | 25.2 MB |
| `neuro-packages` | 7,490 unique<br>(10,420 listed) | **342.5s** | 19,902 | 7,490 | 1,887 | 10,376 / 10,420 | 7,544 | 7,653 | 4.2 MB |
...
Added by `perf/00-scan-status-histogram`. Measured on `neuro-packages` (7,490 unique packages):
...
|---|---|---|---|
| `neuro-containers` | 59 | 59 | **1.00** |
| `neuro-packages` | 1,887 | 1,887 | **1.00** |
...
Measured on `neuro-packages`, calling `_build_graph` directly so the in-memory graph cache cannot mask what is being measured.
...
|---|---|---|---|
| `neuro-packages` | 7,490 | 1,887 | **1.5 MB** |
| `neuro-containers-build` | 18 | **2** | **10.1 MB** |

Two container images outweigh seven thousand packages by 7×: one image carries 1,932 CVE records at ~1,451 bytes each. Expect a `neuro-containers`-scale repo (20,102 CVEs) to sit around 30 MB. Bounded by eviction, but worth knowing before pointing this at a large estate.
